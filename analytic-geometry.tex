\section{Basic Exercises}

\faCalculator \ For each pair of vectors $\vv_1$ and $\vv_2$, determine:
\begin{itemize}
\item the lengths of $\vv_1$ and $\vv_2$;
\item the angle between $\vv_1$ and $\vv_2$;
\item the distance between $\vv_1$ and $\vv_2$;
\item the unit length normalizations of $\vv_1$ and $\vv_2$.
\end{itemize}
You must simplify the expression as much as you can before giving the
approximate result to a couple of decimal places.
\begin{tasks}[style=enumerate](2)
\task
  \begin{displaymath}
    \vv_1 = \vTwo 7 3
    \qquad
    \vv_2 = \vTwo {-1}  5
  \end{displaymath}
\task
  \begin{displaymath}
    \vv_1 = \vThree {-2} 4 {-5}
    \qquad
    \vv_2 = \vThree 2 2 {-2}
  \end{displaymath}
\end{tasks}
\begin{tasks}[resume, style=enumerate](1)
\task
  \begin{displaymath}
    \vv = \vFive 1 {-1} 5 7 4
    \qquad
    \vu = \vFive 3 1 3 {-1} 0
  \end{displaymath}
  Additionally, without doing any further calculations, determine
  approximately the angle between $\vv$ and $\vv - \vu$.  Justify your
  answer.
%% \item
%%   \begin{displaymath}
%%     \vv_1 = \vFive 1 {-1} 5 7 4
%%     \qquad
%%     \vv_2 = \vFive 3 0 3 {-5} {-7}
%%   \end{displaymath}
\end{tasks}
Determine if each of the following sets of vectors is an orthogonal set.
\begin{tasks}[resume, style=enumerate](2)
\task
  \begin{displaymath}
    \left\{
    \vThree 9 {-18} {-18},
    \vThree {-2} {-14} {13}
    \right\}
  \end{displaymath}
\task
  \begin{displaymath}
    \left\{
    \vFour 0 4 4 0,
    \vFour {-2} 2 0 0,
    \vFour 2 1 {-1} {-6}
    \right\}
  \end{displaymath}
\end{tasks}
Express the vector $\vv$ in terms of the given orthonormal basis
$\cB$.
\begin{tasks}[resume, style=enumerate](2)
\task
  \begin{displaymath}
    \vv = \vTwo{-8}{9}
    \qquad
    \cB =
    \left\{
    \vTwo{-\frac{\sqrt{2}}{2}}{\frac{\sqrt{2}}{2}},
    \vTwo{\frac{\sqrt{2}}{2}}{\frac{\sqrt{2}}{2}}
    \right\}
  \end{displaymath}
\task
  \begin{displaymath}
    \vv = \vThree 3 9 {-3}
    \qquad
    \cB =
    \left\{
    \vThree {\frac{\sqrt{2}}{2}} {\frac{\sqrt{2}}{2}} {0},
    \vThree {\frac{\sqrt{6}}{6}} {-\frac{\sqrt{6}}{6}} {-\frac{\sqrt{6}}{3}},
    \vThree {-\frac{\sqrt{3}}{3}} {\frac{\sqrt{3}}{3}} {-\frac{\sqrt{3}}{3}}
    \right\}
  \end{displaymath}
\end{tasks}
Determine the projection of $\vv$ onto the span of the given set of vectors.
\begin{tasks}[resume, style=enumerate](2)
\task
  \begin{displaymath}
    \vv = \vTwo 9 7
    \qquad
    U =
    \left\{
    \vTwo 7 {-9}
    \right\}
  \end{displaymath}
\task
  \begin{displaymath}
    \vv = \left[\begin{matrix}3\\3\\5\end{matrix}\right]
    \qquad
    U =
    \left\{
    \left[\begin{matrix}-2\\0\\-10\end{matrix}\right]
    \right\}
  \end{displaymath}
\task
  \begin{displaymath}
    \vv = \left[\begin{matrix}-3\\6\\2\end{matrix}\right]
    \qquad
    U =
    \left\{
    \left[\begin{matrix}- \frac{\sqrt{2}}{2}\\\frac{\sqrt{2}}{2}\\0\end{matrix}\right],
    \left[\begin{matrix}\frac{2}{3}\\\frac{2}{3}\\- \frac{1}{3}\end{matrix}\right]
    \right\}
  \end{displaymath}
\end{tasks}
Determine an orthogonal diagonalization of the following matrices.
\begin{tasks}[resume, style=enumerate](1)
\task
  \begin{displaymath}
    \begin{bmatrix}
      3 & 1 \\
      1 & 3
    \end{bmatrix}
  \end{displaymath}
\end{tasks}

\section{True/False}

\trueFalseHeader

\begin{tasks}[resume, style=enumerate](1)
\task
  If $\|\vu\|^2 + \|\vv\|^2 = \|\vu + \vv\|^2$, then $\vu$ and $\vv$
  are orthogonal.
\task
  For a square matrix $A$, vectors in $\col A$ are orthogonal to
  vectors in $\nul A$.
\task
  There can be a linear dependence relationship between vectors in an
  orthogonal set.
\task
  For any $A \in \R^{m \times n}$, there is an eigenbasis of $\R^n$
  for $A^TA$.
\task
  In $\R^4$, any set of vectors that has 5 members cannot be an
  orthogonal set.
\task
  Orthogonal matrices are invertible.
\task
  Orthogonal matrices have determinant 1.
\task
  For any matrix $A \in \R^{m \times n}$ and vector $\vv \in \R^n$, if
  $\|A \vv \| = \vzero$ then $\vv \in \nul A$.
\task
  For an $m \times n$ matrix with orthogonal columns, it may be the
  case that $m < n$.
\task
  For any matrix $U \in \mathbb \R^{n \times n}$, if $U$ is orthogonal
  then $\det U = \pm 1$.
\task
  The orthogonal projection of $\vy$ onto $\vv$ is the same as the
  orthogonal projection of $\vy$ onto $c \vv$ whenever $c \neq 0$.
\task
  For any vector $\vy \in \R^n$ and any subspace $W$ of $\R^n$, the
  vector $\vy - \proj_W \vy$ is orthogonal to $W$.
\task
  For any matrix $A \in \R^{n \times n}$, if $A$ has an eigenbasis of
  $\R^n$ then $A$ is symmetric.
\task
  For any matrix $A$ with orthogonal columns, $A^TA = I$.
\task
  Every orthogonal set is linearly independent.
\task
  For any matrix $A$, if $A$ is orthogonally diagonalizable, then so
  is $A^T$.
\task
  $\|A \vv \| = \| A^2 \vv \|$ for any orthogonal matrix $A \in \R^{n
  \times n}$ and any vector $\vv \in \R^n$.
\task
  For any vector $\vu \in \R^n$, if $\|\vu\| = 1$ then the matrix $\vu
  \vu^T$ implements projection onto $\vspan\{\vu\}$.
\end{tasks}

\section{More Difficult Problems}

\begin{tasks}[resume, style=enumerate](1)
\task \hfill

  \begin{center}
    \begin{tikzpicture}[scale=0.4]
      \draw [<->] (10, 0) -- (-10, 0);
      \draw [<->] (0, 10) -- (0 ,-10);
      \draw [->, thick] (0, 0) -- (4, 2);
      \node at (5.4, 2) {(4, 2)};
      \draw [->, thick] (0, 0) -- (-8, 6);
      \node at (-9, 6.6) {(-8, 6)};
      \draw [dotted] (4, 2) -- (-8, 6);
      \draw [->, thick] (0, 0) -- (1, 3);
      \draw (0.842, 2.526) -- (1.316, 2.368);
      \draw (1.316, 2.368) -- (1.474, 2.842);
      \node at (1.2, 3.4) {$\vv$};
      \draw [->, thick] (0, 0) -- (-4, 8);
      \node at (-4, 8.5) {(-4, 8)};
      \draw [dotted] (-8, 6) -- (-4, 8);
      \draw [dotted] (-4, 8) -- (4, 2);
    \end{tikzpicture}
  \end{center}
  \begin{enumerate}[label=(\alph*)]
  \item
    Determine the distance between $(-8, 6)$ and $(4, 2)$.
  \item
    Determine the length of the vector $\vv$, whose endpoint is on the
    line segment between $(-8, 6)$ and $(4, 2)$ and which forms a
    $90^\circ$ angle with that line segment.
  \item
    Use these values to determine the area of the parallelogram formed by
    $(0, 0)$, $(4, 2)$, $(-4, 8)$, and $(-8, 6)$.

  \item Determine a matrix in $\R^{2\times 2}$ that transforms the
    unit square into the parallelogram from the previous part.  Recall
    that the unit square is the set of points in
    \begin{displaymath}
      \{(x, y) : 0 \leq x \leq 1 \text{ and } 0 \leq y \leq 1\}
    \end{displaymath}
  \item Calculate the determinant of the matrix from the previous part.
  \end{enumerate}
\task
  Determine if the following six vectors form an orthogonal set.
  Justify your answer. \textit{Hint:} You do not need to explicitly
  check every pair for orthogonality; you can argue more generally.
  \begin{displaymath}
    \vv_1 = \begin{bmatrix} 1 \\ -2 \\ 1 \\ 0 \\ 0 \\ 0 \end{bmatrix}
    \qquad
    \vv_2 = \begin{bmatrix} 0 \\ 1 \\ 2 \\ 0 \\ 0 \\ 0 \end{bmatrix}
    \qquad
    \vv_3 = \begin{bmatrix} -5 \\ -2 \\ 1 \\ 0 \\ 0 \\ 0 \end{bmatrix}
  \end{displaymath}
  \begin{displaymath}
    \vv_4 = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 3 \\ -3 \\ 0 \end{bmatrix}
    \qquad
    \vv_5 = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 2 \\ 2 \\ -1 \end{bmatrix}
    \qquad
    \vv_6 = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 1 \\ 1 \\ 4 \end{bmatrix}
  \end{displaymath}
\task Project $\vb$ onto $\vspan\{ \va_1, \va_2, \va_3 \}$.
  \begin{displaymath}
    \vb = \vFour 2 5 6 6
    \qquad
    \va_1 = \vFour 1 1 0 {-1}
    \qquad
    \va_2 = \vFour 1 0 1 1
    \qquad
    \va_3 = \vFour 0 {-1} 1 {-1}
  \end{displaymath}
\task
  Show that any $2 \times 2$ rotation matrix $R_\theta$ is an
  orthogonal matrix.
\task
  Show that any $3 \times 3$ rotation matrix $R_x^\theta$,
  $R_y^\theta$, $R_z^\theta$ (about the $x$-axis, $y$-axis, and
  $z$-axis, respectively) is an orthogonal matrix.
\task
  Determine the matrix that implements orthogonal projection onto
  $\vspan\{ [2 \ \ 3 \ \ 1]^T \}$.
\task Determine a matrix (in terms of $\vu_1$ and $\vu_2$) that
  implements orthogonal projection onto $\vspan \{ \vu_1, \vu_2 \}$
  where $\vu_1$ and $\vu_2$ are orthogonal.
\task
  Show that for any nonzero vector $\vv \in \R^n$, the set of vectors
  orthogonal to $\vv$ forms a subspace of $\R^n$.
\task
  Determine a basis for the set of vectors orthogonal to the following
  vector.
  \begin{displaymath}
    \vThree 1 4 {-3}
  \end{displaymath}
\task
  Determine a basis for the set of vectors orthogonal to every vector
  in the solution set of the following linear equation.
  \begin{displaymath}
    2x_1 + 3x_2 - 4x_3 + 5x_4 = 0.
  \end{displaymath}
\task
  Consider the following pair of vectors.
  \begin{displaymath}
    \vu = \vFour 1 2 1 {-2}
    \qquad
    \vv = \vFour {-3} {-6} {-2} 2
  \end{displaymath}
  For this problem, we will be using a theorem that isn't too difficult
  to show: every vector in $\nul A^T$ is orthogonal to every vector in
  $\col A$.  Use this to determine two linearly independent vectors
  with integer entries that are orthogonal to every vector in $\vspan
  \{ \vu , \vv \}$.
\task
  For this problem, we will use another theorem that's more difficult
  to prove: $\rank A + \dim(\nul A^T) = m$ for any matrix $A \in \R^{m
    \times n}$.  Use this to show that $\rank A = \rank A^T$.
\task
  Consider the following vectors.\footnotemark
  \begin{displaymath}
    \vv_1 = \vFour 2 1 0 {-1}
    \qquad
    \vv_2 = \vFour 3 0 0 0
    \qquad
    \vv_3 = \vFour {-4} 9 0 {-11}
    \qquad
    \vu = \vFour 2 {-2} 0 {-5}
  \end{displaymath}
  \begin{enumerate}[label=(\alph*)]
  \item
    Find the component of $\vv_2$ orthogonal to $\vv_1$ (that is, find
    the vector $\vz$ such that $\vv_2 = \hat{\vv_2} + \vz$ where
    $\hat{\vv_2}$ is the orthogonal projection of $\vv_2$ onto
    $\vv_1$). We will refer to this as $\vv_2'$ below.
  \item
    Find the component of $\vv_3$ orthogonal to $\vv_1$. We will refer
    to this as $\vv_3'$ below.
  \item
    Find the component of $\vv_3'$ orthogonal to $\vv_2'$. We will
    refer to this as $\vv_3''$ below.
  \item
    Demonstrate that $\{\vv_1, \vv_2', \vv_3''\}$ is an orthogonal
    set.
  \item
    Find $[\vu]_{\cB}$ where $\cB = \{\vv_1, \vv_2', \vv_3''\}$
    without any row reductions.
  \end{enumerate}
\end{tasks}
\footnotetext{This problem goes over the \textit{Gram-Schmidt
    process}, an algorithm for converting a basis into an orthogonal
  basis.}
\begin{tasks}[resume, style=enumerate](1)
\task
  Repeat the previous problem with the following vectors.
  \begin{displaymath}
    \vv_1 = \vFour 1 0 1 1
    \qquad
    \vv_2 = \vFour 0 1 1 2
    \qquad
    \vv_3 = \vFour 1 0 2 {-2}
    \qquad
    \vu = \vFour2 {-2} 1 {-5}
  \end{displaymath}
\task
  \begin{displaymath}
    A =
    \begin{bmatrix}
      1 & 2 & -5 \\
      1 & 1 & 3 \\
      8 & 14 & -6 \\
      1 & 2 & 1
    \end{bmatrix}
    \qquad
    B =
    \begin{bmatrix}
      1 & -4 & -2 & 9 & 0 \\
      0 & 2 & 2 & -6 & 1 \\
      3 & -5 & 1 & 9 & 5 \\
      0 & 1 & 1 & -2 & 1
    \end{bmatrix}
  \end{displaymath}
  \begin{enumerate}[label=(\alph*)]
  \item
    \faCalculator \ Determine approximately the matrix that implements
    orthogonal projection onto $\col A$.
  \item
    \faCalculator \ Determine approximately the matrix which
    implements orthogonal projection onto $\col B$.  \textit{Hint:}
    First determine $\rank B$.
  \item
    Compare the matrices from the previous two parts.  Explain what
    this implies about the relationship between $\col A$ and $\col B$.
    Justify your answer.
  \end{enumerate}
\task
  Consider the following linear equation
  \begin{displaymath}
    x_1 - x_2 + x_3 = 0
  \end{displaymath}
  and vector
  \begin{displaymath}
    \vv = \vThree 4 1 0
  \end{displaymath}

  \begin{enumerate}[label=(\alph*)]
  \item
    Determine a vector $\vz$ which is orthogonal to the plane given by
    the above linear equation.
  \item
    Determine a basis $\{\vb_1, \vb_2\}$ for the plane given by the
    above linear equation.
  \item
    Determine a solution to the vector equation $y_1\vz + y_2\vb_1 +
    y_3\vb_2 = \vv$.
  \item
    Using the previous part, determine the orthogonal projection of
    $\vv v$ onto the plane given by the above linear equation.
    Justify your answer.
  \end{enumerate}
\task
  One matrix factorization not covered in this course is the
  \textbf{QR factorization}: any square matrix $A \in \R^{n \times n}$
  can be expressed as a matrix product $A = QR$ where $Q \in \R^{n
  \times n}$ has orthonormal columns and $R \in \R^{n \times n}$ is
  upper triangular (i.e., has zeros below the diagonal).  Let
  \begin{displaymath}
    A = \begin{bmatrix} \va_1 & \va_2 \end{bmatrix}
    =
    \begin{bmatrix} 2 & -3 \\ 1 & 1 \end{bmatrix}
  \end{displaymath}
  \begin{enumerate}[label=(\alph*)]
  \item
    Determine an orthonormal basis $\{\vb_1, \vb_2\}$ for $\col A$
    such that $\vb_1$ is a scalar multiple of $\va_1$.
  \item
    Determine an upper triangular matrix $R$ (i.e., a matrix of the
    form $\begin{bmatrix} a & b \\ 0 & c \end{bmatrix}$) such that $A =
    \begin{bmatrix} \vb_1 & \vb_2 \end{bmatrix} R$, where $\vb_1$ and
    $\vb_2$ are the vectors you determined in the previous part.
    \textit{Hint:} Think about the relationship between $\vb_2$ and
    $\proj_{\vspan\{\va_1\}} \va_2$.
  \end{enumerate}
\end{tasks}

\section{Challenge Problems}

\begin{tasks}[resume, style=enumerate](1)
\task
  Show that the row vectors of an orthogonal matrix form an
  orthonormal set.
\task
  Show that the product of two orthogonal matrices is orthogonal.
\task
  Show that $\vv \in \nul A^T$ if and only if $\vv$ is orthogonal to
  every vector in $\col A$ for any matrix $A \in \R^{m \times n}$ and
  vector $\vv \in \R^m$.
\task
  Show that $\rank A + \dim(\nul A^T) = m$ for any matrix $A \in \R^{m
    \times n}$.
\end{tasks}
