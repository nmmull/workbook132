...
\task
  Every stochastic matrix is invertible.
\task
...
\task
  ...
  is a diagonal matrix $D$ such that $AD$ is stochastic.
\task
  For any stochastic matrix $T \in \R^{n \times n}$ and probability
  vector $\vv \in \R^n$, if $T$ is not regular, then $\lim_{k \to
  \infty} T^k \vv$ does not converge.
\end{tasks}
...
\task
  ...
  \item
    ...
    down to \textbf{LOW}.
  \item
    If the price is \textbf{LOW}, there is a 60\% chance it will
  ...
  term, is it more likely to remain \textbf{STEADY}, go up \textbf{HIGH},
  or dip down \textbf{LOW}?  Justify your answer.
\task
  Suppose that we're trying to predict the performance of the $A_5$
  ...
  \item
    If they win their first game, how should we expect them to
...
\task
  ...
  \item
    Determine if $T$ has a unique steady-state distribution.  Justify
    your answer.
  \end{enumerate}
\task
  Consider the following state diagram.
...
\task
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[circle, draw] (2) [below left=of 1] {2};
      \node[circle, draw] (3) [below right=of 1] {3};
      \draw[->] (1) edge[loop above, "{$1 - p$}"] (1);
      \draw[->] (2) edge["{$p$}"] (1);
      \draw[->] (1) edge["{$p$}"] (3);
      \draw[->] (2) edge[loop left, "{$0.5 - p$}"] (2);
      \draw[->] (2) edge[bend left, "{$0.5$}"] (3);
      \draw[->] (3) edge[bend left, "{$0.7$}"] (2);
      \draw[->] (3) edge[loop right, "{$0.3$}"] (3);
    \end{tikzpicture}
  \end{center}
  The transitions out of state $1$ should appear in the first column
  of the transition matrix, out of state $2$ in the second column,
  and out of state $3$ in the third column.
  \begin{enumerate}[label=(\alph*)]
  \item
    Determine the transition matrix $A$ of the above state diagram in
    terms of $p$.
  \item
    Is the matrix $A$ regular in the case that $p = 0$?  Justify your
    answer.
  \item
    Determine $\lim_{k \to \infty} A^k \vThree {0.2} {0.4} {0.4}$ in
    the case that $p = 0.1$.
  \item
    Determine $\lim_{k \to \infty} A^k \vThree {0.2} {0.4} {0.4}$ in
    the case that $p = 0$.
...
\task
 Let $T$ denote the transition matrix of the following state diagram.
  ...
  \item
    ...
    symmetric and, hence, orthogonally diagonalizable.\footnotemark
  \item
    Give a closed-form solution for $T^k [(1-q) \ \ q]^T$ in terms
    of $p$, $q$ and $k$.
  \end{enumerate}
\end{tasks}
\footnotetext{The second largest eigenvalue tells us about the
  \textit{rate of convergence} to the steady-state distribution. The
  smaller $|\lambda_2|$ is, the faster the convergence to the
